/**
 * Cross-Domain Logout Utility for Dashboard
 * Ensures user is logged out from all domains simultaneously
 */

/**
 * Perform logout across all Vikareta domains from dashboard
 */
export async function logoutFromAllDomains(): Promise<void> {
  ...
  console.log('Cross-domain logout completed for all domains from dashboard');
}
...
export async function performSecureLogout(): Promise<void> {
  try {
    ...
    console.error('Dashboard secure logout error:', error);
